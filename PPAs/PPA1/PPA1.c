...
int exitChar;
char cmd;
char contents[PRINTSIZE] = ">><<                "; //i dont know why but it works when like this instead of just ">><<", the spaces need to be initiliased to for some reason
int startPos;
int lastIndex; // the index of the current first '<' and where the new char will be inserted
int i;
char toggleDir; //the char responsible for toggling the direction that the contents roll in
int direction; //each time toggleDir pressed, this value is *-1. + = R to L, - = L to R
...
void Initialize(void) {
    MacUILib_init(); //Call the MacUI Library Initialization Function 
    // Initialize variables
    exitFlag = 0;   // 0 - do not exit, non-zero - exit the program
    exitChar = 27;  // int 27 = char 'esc'
    toggleDir = 9;
    cmd = 0;        // NULL char
    startPos = 0;   // index of the first char in 'contents' to be printed first in dynamic contents
    lastIndex = 2;  // the index of the current first '<' and where the new char will be inserted
    i=0;
    direction = 1;  
...
//OVERWHELMING AMOUNT OF COMMENTS THAT WERE INSIDE 'RunLogic' 
...
void RunLogic(void) { // [TODO]: Implement the above pseudocode logic
    ...
    if (cmd == exitChar){ //if 'esc' then press enter to shutdown message will appear
        exitFlag = 1; 
    }
    ...
    //the last section of the statement respectively includes: ' ' ! ' , . ? 
    else if ((cmd >= '0' && cmd <= '9') || (cmd >= 'A' && cmd <= 'Z') || (cmd >= 'a' && cmd <= 'z')|| cmd == 32 || cmd == 33 || cmd == 39 || cmd == 44 || cmd == 46 || cmd == 63){
        InsertChar();
        cmd = 0; 
    }
...
void DrawScreen(void) {
    ...
    if (direction == 1){
        ...
        for (i=PRINTSIZE-1; i >=0; i--){
            MacUILib_printf("%c", (contents[(startPos + i) % PRINTSIZE]));
        }
    }


    // Static Contents - Bottom
    MacUILib_printf("\n====================\n");
...
// 2. The main logic is not executed too often, such that the game won't act too ahead of the player's reaction.  
// Otherwise, the game will be impossible to be beaten.
void LoopDelay(void) {
    ...
    MacUILib_Delay(150000); //0.15s --> just the right delay for no 'glitch' effect, visually pleasing, and easy to read
}
...
void InsertChar(void){ //inserts a char before the 
   ...
   int offset = 2;
    ...
    if (lastIndex < (PRINTSIZE - offset)){
        contents[lastIndex+offset] = '<';
        contents[lastIndex] = cmd;
        lastIndex++;
    }
    if(direction==-1){
        FlipContents();
...
void FlipContents(void){
    int mid = PRINTSIZE/2; //the middle position of contents
    int i, temp; //no need to declare global these since they're only needed here during thes function, otherwise takes up unnecessary space

    for (i = 0; i < mid; i++){
        temp = contents[mid-i-1];
        contents[mid-i-1] = contents[mid+i];
        contents[mid+i] = temp;
    }
}